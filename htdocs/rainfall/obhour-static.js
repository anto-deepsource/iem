...
Ext.onReady(() => {
  defaultnetwork =
    new URL(window.location).searchParams.get("network") || "IA_ASOS";
  ...
  const task = {
    run() {
      ...
      Ext.getCmp("precipgrid")
        .setTitle(
          `Precip Accumulation valid at ${localDate.format("d M Y h A")}`,
        )
        .getStore()
        .load({
  ...
  function updateHeaders(ts) {
    ...
    for (let i = 2; i < cm.getColumnCount(); i++) {
      ...
      if (col.toffset === 0) {
        ...
        cm.setColumnHeader(
          i,
          `Midnight<br />${ts0.format("m/d hA")}<br />${ts.format("m/d hA")}`,
        );
      } else {
        cm.setColumnHeader(
          i,
          `${offset_render(col.toffset)}<br />${ts0.format("m/d hA")}<br />${ts.format("m/d hA")}`,
        );
      }
    }
  }
