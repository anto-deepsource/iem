...
function updateSliderLabels() {
  ...
  const yearLabelText = document.querySelector(
    'label[for="year_slider"] .year-label-text',
  );
  const dayLabelText = document.querySelector(
    'label[for="day_slider"] .day-label-text',
  );
  const hourLabelText = document.querySelector(
    'label[for="hour_slider"] .hour-label-text',
  );
  const minuteLabelText = document.querySelector(
    'label[for="minute_slider"] .minute-label-text',
  );
  if (yearLabelText) {
...
function update() {
  ...
  if (hourSlider?.noUiSlider) {
    if (interval >= 1440) {
      // Disable if interval is a day or more
  ...
  const url = templateText
    .replace(/%Y/g, dt.utc().format("YYYY"))
...
function updateUITimestamp() {
  const opt = getSelectedOption();
  if (opt.getAttribute("data-interval") >= 1440) {
    document.getElementById("utctime").textContent = dt
      .utc()
      .format("MMM Do YYYY");
    document.getElementById("localtime").textContent = dt
      .local()
      .format("MMM Do YYYY");
  } else {
    document.getElementById("utctime").textContent = dt
...
function buildUI() {
  ...
  const updateDaySlider = () => {
    ...
    noUiSlider.create(daySlider, {
      ...
      tooltips: {
        to: (value) =>
          moment([year]).dayOfYear(Math.round(value)).format("MMM D"),
        from: (value) => Math.round(value),
      },
      format: {
        to: (value) => Math.round(value),
  ...
  document
    .querySelector("select[name=products]")
