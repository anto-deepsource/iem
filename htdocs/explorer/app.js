...
function mapClickHandler(event) {
  ...
  const $newdiv = $(`.${prefix}-data-template`)
    .clone()
    .css("display", "block")
    .appendTo($(div));
...
function displayFeature(evt) {
  const features = olMap.getFeaturesAtPixel(
...
function addButtons(dlg) {
  ...
  const $min = $("<button>", {
    class:
      "ui-button ui-corner-all ui-widget ui-button-icon-only ui-window-minimize",
    type: "button",
    title: "Minimize",
  }).insertBefore($close);
  ...
  const $max = $("<button>", {
    class:
      "ui-button ui-corner-all ui-widget ui-button-icon-only ui-window-maximize",
    type: "button",
    title: "Maximize",
  }).insertBefore($close);
