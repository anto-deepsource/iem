...
function init_map(idx, inst) {
  ...
  inst.map.on("click", (evt) => {
    const feature = inst.map.forEachFeatureAtPixel(
      evt.pixel,
      (feature2, _layer) => {
        return feature2;
      },
  ...
  inst.vectorLayer.getSource().on("change", (_e2) => {
    if (
      inst.vectorLayer.getSource().getState() === "ready" &&
      inst.zoomReset === false
    ) {
      inst.map.getView().fit(inst.vectorLayer.getSource().getExtent(), {
    ...
    $(inst.select).append('<option value="id">ID</option>');
    const data = [];
    // If we have a column order, use it!
    if (inst.proporder) {
      inst.proporder.forEach((val) => {
        columns.push({ title: val, data: val });
        $(inst.select).append(`<option value="${val}">${val}</option>`);
      });
    }
    inst.vectorLayer
      .getSource()
      ...
      .forEach((feat) => {
        if (columns.length === 1) {
          feat.getKeys().forEach((key) => {
            ...
            $(inst.select).append(`<option value="${key}">${key}</option>`);
