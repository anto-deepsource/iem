...
$().ready(() => {
  ...
  $("form[name='iemss'] :submit").click(() => {
    ...
    if ($("#iemss").data("supports-all") !== 0) {
      ...
      if (
        $("#stations_out option").length >= geojsonSource.getFeatures().length
      ) {
  ...
  $("#iemss-metadata-link").attr(
    "href",
    `/sites/networks.php?network=${network}`,
  );

  $("#stations_in").dblclick(() => {
    return !$("#stations_in option:selected")
      .remove()
      .appendTo("#stations_out");
  });
  $("#stations_out").dblclick(() => {
    return !$("#stations_out option:selected")
      .remove()
      .appendTo("#stations_in");
  });
