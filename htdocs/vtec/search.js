...
function updateTable2ByUGC() {
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      $.map(data.events, (row) => {
        const uri = `<a href="${row.url}" target="_blank">${row.eventid}</a>`;
        table2.row.add([uri, row.ph_name, row.sig_name, row.issue, row.expire]);
      });
      table2.draw();
    },
  });
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      $.map(data.events, (row) => {
        ...
        table3.row.add([uri, row.wfo, row.locations, row.issue, row.expire]);
      });
      table3.draw();
    },
  });
...
function buildUI() {
  ...
  $(".iemtool").click(function () {
    ...
    if (btn.data("table") === 2) {
      ...
      params.sdate = $.datepicker.formatDate(
        DATE_FMT,
        sdate.datepicker("getDate"),
      );
      params.edate = $.datepicker.formatDate(
        DATE_FMT,
  ...
  table2 = $("#table2").DataTable({
    language: {
      emptyTable:
        "Drag marker on map or select UGC to auto-populate this table",
    },
  });
