...
function updateTitle() {
  $("#maptitle").text(
    `The map is displaying ${$("#varpicker :selected").text()} valid at ${currentdt}`,
  );
  if (timeChanged) {
  ...
  invgj = new ol.layer.Vector({
    ...
    style(feature) {
      ...
      $(`#${feature.getId()}_arrow`).attr(
        "src",
        feature.get("is_inversion")
          ? "/images/red_arrow_down.svg"
          : "/images/green_arrow_up.svg",
      );
...
function setupUI() {
  $(".dt").click((event) => {
    ...
    currentdt = new Date(
      currentdt.valueOf() + parseInt($(event.target).data("delta")),
    );
    setDate();
    updateMap();
  });
