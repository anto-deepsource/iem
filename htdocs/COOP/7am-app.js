...
function updateURL() {
  const tt = $.datepicker.formatDate(
    "yymmdd",
...
function updateDate() {
  ...
  const fullDate = $.datepicker.formatDate(
    "yy-mm-dd",
    $("#datepicker").datepicker("getDate"),
  );
...
function makeVectorLayer(dt, title, group) {
  return new ol.layer.Vector({
    ...
    style(feature) {
      let txt =
        feature.get(renderattr) === 0.0001 ? "T" : feature.get(renderattr);
...
$(document).ready(() => {
  ...
  cocorahsLayer = makeVectorLayer(
    $.datepicker.formatDate("yy-mm-dd", new Date()),
    "IA CoCoRaHS Reports",
    "cocorahs",
  );

  coopLayer = makeVectorLayer(
    $.datepicker.formatDate("yy-mm-dd", new Date()),
    "NWS COOP Reports",
    "coop",
  );
  azosLayer = makeVectorLayer(
    $.datepicker.formatDate("yy-mm-dd", new Date()),
    "ASOS/AWOS Reports",
    "azos",
  );
