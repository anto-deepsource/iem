...
let nexradBaseTime = moment()
  .utc()
  .subtract(moment().minutes() % 5, "minutes");
  ...
  style(feature) {
    ...
    if (mag !== "") {
      ...
      textStyle
        .getText()
        .getBackgroundFill()
        .setColor(lsrTextBackgroundColor[typ] || "black");
      return textStyle;
    }
...
lsrLayer.getSource().on("change", () => {
  ...
  if (lsrLayer.getSource().getState() === "ready") {
    olmap.getView().fit(lsrLayer.getSource().getExtent(), {
      size: olmap.getSize(),
      padding: [50, 50, 50, 50],
    });
  }
...
sbwLayer.getSource().on("change", () => {
  ...
  sbwtable
    ...
    .sort()
    .each((d) => {
      sbwtypefilter.append(
        `<option value="${iemdata.vtec_phenomena[d]}">${iemdata.vtec_phenomena[d]}</option`,
      );
    });
});
...
function formatSBW(feature) {
  ...
  const ss =
    sig in iemdata.vtec_significance ? iemdata.vtec_significance[sig] : sig;
    ...
    columnDefs: [
      {
        targets: 3,
        render(data) {
          return data in iemdata.vtec_phenomena
            ? iemdata.vtec_phenomena[data]
            : data;
        },
      },
      {
        targets: 4,
        render(data) {
          return data in iemdata.vtec_significance
            ? iemdata.vtec_significance[data]
            : data;
        },
      },
      {
