...
function liveShot() {
  ...
  $("#webcam_image").attr(
    "src",
    `/current/live/${currentCameraFeature.get("cid")}.jpg?ts=${ts.getTime()}`,
  );
  aqlive = false;
}
...
function updateHashLink() {
  ...
  const extra = realtimeMode
    ? ""
    : `/${$("#dtpicker").data("DateTimePicker").date().utc().format(ISOFMT)}`;
...
function doRWISView() {
  ...
  while (i < 10) {
    ...
    if (url !== null && url !== undefined) {
      ...
      $("#rwislist").append(
        `<div class="col-md-2"><img onclick="handleRWISClick(this);" src="${url}" class="img img-responsive"></div>`,
      );
    }
    i += 1;
  }
...
function parseURI() {
  ...
  if (tokens.length === 2) {
    ...
    if (tokens2.length === 1) {
      ...
      $("#dtpicker")
        .data("DateTimePicker")
        .date(moment(escapeHTML(tokens2[1])));
    }
  }
}
...
function popupSBW(feature) {
  const content =
    `<strong>You clicked:</strong> ${feature.get("wfo")} ` +
  ...
  webcamGeoJsonLayer = new ol.layer.Vector({
    ...
    style: (feature) => {
      if (
        ...
        cameraStyle2
          .getImage()
          .setRotation((parseInt(feature.get("angle")) / 180.0) * 3.14, 10);
        return [cameraStyle2];
      }
      cameraStyle
        .getImage()
